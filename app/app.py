...
def train_model(df):
    try:
        ...
        features = ['dayofweek', 'month', 'year'] + [f'lag_{lag}' for lag in [1, 2, 3, 7, 14, 30]]
        target = 'target'
        ...
        # Save model in both XGB and PKL formats
        model.save_model("trained_model.xgb")
                ...
                if st.button("Predict"):
                    if 'model' in st.session_state:
                        prediction = st.session_state['model'].predict(
                            df[df['date'] == pd.to_datetime(forecast_date)][
                                ['dayofweek', 'month', 'year'] + [f'lag_{lag}' for lag in [1, 2, 3, 7, 14, 30]]
                            ]
                        )
